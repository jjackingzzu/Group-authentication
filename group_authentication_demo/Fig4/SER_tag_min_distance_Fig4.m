...
% System parameters
theta_0 = (0:1:180)/180*pi;                         % Rotation angle
...
% error storage
SER_message = zeros(length(K_0), length(theta_0));
SER_tag = zeros(length(K_0), length(theta_0));
...
for index_K = 1:length(K_0)
    ...
    for index_theta = 1:length(theta_0)
        ...
        % tag SER over Monte Carlo trials
        error_message(index_K,index_theta,:) = error_message_temp;
        error_tag(index_K,index_theta,:) = error_tag_temp;
        SER_message(index_K,index_theta) = sum(error_message_temp) / (run_times * message_length);
        SER_tag(index_K,index_theta)=sum(error_tag_temp)/message_length/run_times;
...
d_min_all = zeros(length(K_0), length(theta_0));
...
for index_K = 1:length(K_0)
    ...
    for index_theta = 1:length(theta_0)
        ...
        Constellation_sum_rotated = Constellation_sum * exp(-1i*theta);
        C = [Constellation_sum; Constellation_sum_rotated];

        d_min_all(index_K,index_theta) = minConstellationDistance(C);

    end
end


% Plot tag SER and minimum constellation distance versus rotation angle
theta_plot = theta_0 / pi * 180;
panel_label = {'(a)','(b)','(c)','(d)'};
...
for index_K = 1:length(K_0)
    ...
    yyaxis left
    semilogy(theta_plot,SER_tag(index_K,:),'-b','LineWidth',1);
    xlim([1 180]);
    ylim([min(SER_tag(index_K,:)) max(SER_tag(index_K,:))]);
    xlabel('$\theta$','FontSize',14,'FontName','Times New Roman','Interpreter','latex');
    ylabel('SER','FontSize',14,'FontName','Times New Roman','Interpreter','tex');

    yyaxis right
    plot(theta_plot,d_min_all(index_K,:),'-r','LineWidth',1);
    ylabel('Minimum Distance','FontSize',14,'FontName','Times New Roman','Interpreter','none');
    ylim([min(d_min_all(index_K,:)) max(d_min_all(index_K,:))]);
...
%exportgraphics(gcf,'SER_tag_min_distance_Fig4.pdf','Resolution',600);