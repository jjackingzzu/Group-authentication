...
% SLO-PLA: multiple users cooperatively embed one group tag through
% power-level modulation and perform unified group authentication.
% reference: Slope Authentication at the Physical Layer 
...
message_fail_rate = cell(1,length(K_0));
authentication_fail_rate = cell(1,length(K_0));
...
for index_K = 1:length(K_0)
    ...
    message_fail_rate{index_K} = zeros(length(rho_0),length(M_list));
    authentication_fail_rate{index_K} = zeros(length(rho_0),length(M_list));
    ...
    for index_RF = 1:length(M_list)
        ...
        for index_rho = 1:length(rho_0)
            ...
            parfor times = 1:run_times
                ...
                tx_symbol = data_symbol * sqrt(1-rho) .* (1-tag_bit) ...
                          + data_symbol * sqrt(1+rho) .* tag_bit;
                ...
                % Message detection
                % d_message = abs(R_12 - Constellation_sum);
...
% Tag detection by power projection
projection_power = real(R_12 .* conj(data_estimated_sum)) ...
                 ./ max(abs(data_estimated_sum),eps);

tag_hat = projection_power > abs(data_estimated_sum);
...
error_tag_temp(times) = nnz(tag_hat ~= tag_bit_vec);
            ...
            message_fail_rate{index_K}(index_rho,index_RF) = mean(message_fail);
            authentication_fail_rate{index_K}(index_rho,index_RF) = mean(authentication_fail);
