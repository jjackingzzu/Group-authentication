...
% MTE-PLA
% Multiple users independently transmit their own digital tags and are authenticated separately.
% Overall authentication succeeds only when all K users are authenticated.
% The tag constellation group is identical to the message constellation group.
% Reference: Physical Layer Authentication for Non-Coherent Massive SIMO-Enabled Industrial IoT Communications
...
rho_cell = {
    0.002:0.002:0.08, ...
    0.002:0.002:0.02, ...
    [0.0005:0.0005:0.0015 0.002:0.002:0.01], ...
    [0.0001:0.0001:0.001 0.002:0.002:0.04] ...
};
...
eta_user_cell = cell(1,length(K_0));
...
for index_K = 1:length(K_0)
    ...
    % Per-user false alarm target so that group false alarm <= Pf_target
    Pf_user_target = Pf_target^(1/K);
    L = message_length;
    ...
    eta_user = find(pf_cdf <= Pf_user_target,1,'last') - 1;
    Pf_user_actual = pf_cdf(eta_user+1);
    Pf_group_actual = Pf_user_actual^K;

    eta_user_cell{index_K} = eta_user;

    fprintf('K = %d, Pf target = %.3e, eta user = %d, Pf group actual = %.3e\n', ...
            K, Pf_target, eta_user, Pf_group_actual);

    Constellation_group_selection;

    Constellation_sum = Constellation_table(:,K+1);
    Constellation_sum_tag = Constellation_sum;
    ...
    for index_RF = 1:length(M_list)
        ...
        for index_rho = 1:length(rho_0)
            ...
            parfor times = 1:run_times
                ...
                tag_symbol = tag_bit .* Constellation_table(end,1:K).' ...
                           + (1-tag_bit) .* Constellation_table(1,1:K).';
            ...
            % Each user is authenticated separately; group authentication
            % succeeds only if all K users pass.
            authentication_fail = any(error_tag_user_temp > eta_user,2);
